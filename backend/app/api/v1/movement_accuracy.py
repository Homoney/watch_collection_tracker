...
async def get_current_atomic_time(
    tz: str = Query("UTC", description="Timezone for atomic time (e.g., UTC, America/New_York)")
):
    """
...
@router.post("/{watch_id}/accuracy-readings", response_model=MovementAccuracyReadingResponse, status_code=status.HTTP_201_CREATED)
async def create_accuracy_reading(
    ...
    existing_count = db.query(func.count(MovementAccuracyReading.id)).filter(
    ...
    if existing_count == 0:
        ...
        most_recent_initial = db.query(MovementAccuracyReading).filter(
            MovementAccuracyReading.watch_id == watch_id,
            MovementAccuracyReading.is_initial_reading == True,
            MovementAccuracyReading.reference_time < reference_time
        ).order_by(desc(MovementAccuracyReading.reference_time)).first()
...
@router.get("/{watch_id}/accuracy-readings", response_model=List[MovementAccuracyReadingWithDrift])
def list_accuracy_readings(
    ...
    readings = db.query(MovementAccuracyReading).filter(
        MovementAccuracyReading.watch_id == watch_id
    ).order_by(desc(MovementAccuracyReading.reference_time)).all()
    ...
    for reading in readings:
        ...
        if not reading.is_initial_reading:
            ...
            paired_initial = db.query(MovementAccuracyReading).filter(
                MovementAccuracyReading.watch_id == watch_id,
                MovementAccuracyReading.is_initial_reading == True,
            ...
            if paired_initial:
                try:
                    ...
                    hours_elapsed = (reading.reference_time - paired_initial.reference_time).total_seconds() / 3600
...
@router.get("/{watch_id}/accuracy-readings/{reading_id}", response_model=MovementAccuracyReadingWithDrift)
def get_accuracy_reading(
    ...
    # Get the reading
    reading = db.query(MovementAccuracyReading).filter(
    ...
    if not reading:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Accuracy reading not found"
        )
...
@router.put("/{watch_id}/accuracy-readings/{reading_id}", response_model=MovementAccuracyReadingResponse)
def update_accuracy_reading(
    ...
    # Get the reading
    db_reading = db.query(MovementAccuracyReading).filter(
        MovementAccuracyReading.id == reading_id,
        MovementAccuracyReading.watch_id == watch_id
    ).first()
...
@router.delete("/{watch_id}/accuracy-readings/{reading_id}", status_code=status.HTTP_204_NO_CONTENT)
...
def get_accuracy_analytics(
    ...
    # Get all readings
    all_readings = db.query(MovementAccuracyReading).filter(
        MovementAccuracyReading.watch_id == watch_id
    ).order_by(MovementAccuracyReading.reference_time).all()
    ...
    if len(all_readings) >= 2:
        days_range = (all_readings[-1].reference_time - all_readings[0].reference_time).days
