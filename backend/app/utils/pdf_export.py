...
def generate_watch_pdf(watch_data: dict, storage_path: str = "/app/storage") -> BytesIO:
    ...
    if images:
        primary_image = next(
            (img for img in images if img.get("is_primary")), images[0]
        )
        ...
        if os.path.exists(image_path):
            try:
                img = Image(
                    image_path, width=4 * inch, height=4 * inch, kind="proportional"
                )
                img.hAlign = "CENTER"
                elements.append(img)
    ...
    if watch_data.get("movement_type"):
        basic_data.append(
            ["Movement Type:", watch_data["movement_type"].get("name", "N/A")]
        )
    if watch_data.get("condition"):
        basic_data.append(
            ["Condition:", watch_data["condition"].replace("_", " ").title()]
        )
    ...
    if any(
        [
            watch_data.get("case_diameter"),
            watch_data.get("case_thickness"),
            watch_data.get("lug_width"),
            watch_data.get("water_resistance"),
        ...
        if watch_data.get("water_resistance"):
            spec_data.append(
                ["Water Resistance:", f"{watch_data['water_resistance']} m"]
            )
    ...
    purchase_data = [
        ...
        [
            "Purchase Price:",
            format_currency(
                watch_data.get("purchase_price"),
                watch_data.get("purchase_currency", "USD"),
            ),
        ],
    ]
        ...
        for service in service_history:
            service_data.append(
                [
                    ...
                    format_currency(
                        service.get("cost"), service.get("cost_currency", "USD")
                    ),
                ]
            )

        service_table = Table(
            service_data, colWidths=[1.5 * inch, 1.8 * inch, 1.5 * inch, 1.2 * inch]
        )
    ...
    for i, watch in enumerate(watches):
        ...
        if images:
            ...
            image_path = os.path.join(
                storage_path, "uploads", primary_image["file_path"]
            )

            if os.path.exists(image_path):
                try:
                    img = Image(
                        image_path, width=3 * inch, height=3 * inch, kind="proportional"
                    )
                    img.hAlign = "CENTER"
                    elements.append(img)
                    elements.append(Spacer(1, 0.2 * inch))
        ...
        if watch.get("purchase_price"):
            watch_data.append(
                [
                    "Purchase Price:",
                    format_currency(
                        watch["purchase_price"], watch.get("purchase_currency", "USD")
                    ),
                ]
            )
            ...
            watch_data.append(
                [
                    ...
                    format_currency(
                        ...
                        watch.get("current_market_currency", "USD"),
                    ),
                ]
            )
